...
class sfValidatedFileCustom extends sfValidatedFile
{
	...
	public function save($file = null, $fileMode = 0666, $create = true, $dirMode = 0777)
	{
		if (is_null($file))
		{
			$file = sfContext::getInstance()->getUser()->getUserName().'.png';
		...
		if ($file[0] != '/' && $file[0] != '\\' && !(strlen($file) > 3 && ctype_alpha($file[0]) && $file[1] == ':' && ($file[2] == '\\' || $file[2] == '/')))
		{
			...
			$smallFile = $this->path.DIRECTORY_SEPARATOR.'small/'.$file;
                        
                        echo 
			$file = $this->path.DIRECTORY_SEPARATOR.'normal/'.$file;
		}
		...
		$thumbnail = new sfThumbnail(60, 75, true, true, 85, 'sfGDAdapter');
		$thumbnail->loadFile($this->getTempName());
		$thumbnail->save($smallFile, 'image/jpeg');

		$thumbnail = new sfThumbnail(336, 336, true, true, 85, 'sfGDAdapter');
		$thumbnail->loadFile($this->getTempName());
